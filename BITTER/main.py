...
def show_menu(current_user: str) -> None:
    print("\nWelcome to the bitter, where u can write posts!")
    print(f"You are signed in as: {current_user}")
    print("1. Create post")
    print("2. Look all posts")
...
def main() -> None:
    ...
    while True:
        ...
        choice = input('What do u want to do: ').strip()
        if choice in ('1', 'create post'):
            content = input("Write ur post here: ").strip()
            if create_post(current_user, content):
                print("Ur post was created")
            else:
                print("Could not create post. Check the post lenght")
            #create_post(current_user, content)
        elif choice in ('2', 'Look all posts'):
            for post in posts:
